...
// Package mx provides the interface for building
// [single-image distributed programs].
//
// A program is composed of a set of Go interfaces called
// components. Components are recognized by "mx generate" (typically invoked
// via "go generate"). "mx generate" generates code that allows a component
// to be invoked over the network. This flexibility allows MX
// to decompose the program execution across many processes and machines.
//
// [single-image distributed programs]: https://mx.dev
package mx
...
//go:generate ./cmd/mx/mx generate . ./internal/tool/multi
...
// HealthzURL is the URL path on which MX performs health checks.
...
//	})
//
// As a convenience, MX registers HealthzHandler under
...
// (i.e. T is a struct that embeds mx.Implements[mx.Main]).
...
//
// The application is composed of a set of components that include mx.Main
// as well as any components transitively needed by mx.Main. An instance
// that implement mx.Main is automatically created by mx.Run and passed
// to app. Note: other replicas in which mx.Run is called may also create
// instances of mx.Main.
//
// The type T must be a struct type that contains an embedded
// `mx.Implements[mx.Main]` field. A value of type T is created,
...
//
// If this process is hosting the `mx.Main` component, Run will call app
...
func runLocal[T any, _ PointerToMain[T]](ctx context.Context, app func(context.Context, *T) error) error {
	// Read config from MX_CONFIG env variable, if non-empty.
	opts := mx.SingleMXNOptions{}
	if filename := os.Getenv("MX_CONFIG"); filename != "" {
	...
	wlet, err := mx.NewSingleMXN(ctx, regs, opts)
	if err != nil {
		return err
	}
...
func runRemote[T any, _ PointerToMain[T]](ctx context.Context, app func(context.Context, *T) error, bootstrap runtime.Bootstrap) error {
	...
	opts := mx.RemoteMXNOptions{}
	wlet, err := mx.NewRemoteMXN(ctx, regs, bootstrap, opts)
	if err != nil {
		return err
	}

	// Return when either (1) the remote mxn exits, or (2) the user
...
var (
	// Equivalence checks with the struct in internal/mx/types.go.
	_ = MXInfo(mx.MXInfo{})
	_ = mx.MXInfo(MXInfo{})
)
...
func (i Implements[T]) MX() MXInfo {
	return MXInfo(*i.mxInfo)
}

func (i *Implements[T]) setMXInfo(info *mx.MXInfo) {
	i.mxInfo = info
}

// implements is a method that can only be implemented inside the mx
...
// struct. T must be a component type. MX will automatically
...
// component implementation struct. Once placed, MX automatically
...
// adding a `mx:"name"` struct tag to their listener fields, e.g.:
//
//	type myComponentImpl struct {
//	    mx.Implements[MyComponent]
//	    myListener      mx.Listener
...
// renamed using the `mx:"name"` struct tag.
//
...
// implementation. The MX runtime will take per-component
...
//
// To associate a router with its component, embed [mx.WithRouter] in the
...
// # Semantics
//
// NOTE that routing is done on a best-effort basis. MX will try to
...
// See [Unrouted].
type if_youre_seeing_this_you_probably_forgot_to_run_mx_generate struct{}

// See [Unrouted].
func (implementsImpl) routedBy(if_youre_seeing_this_you_probably_forgot_to_run_mx_generate) {}

// Unrouted is the interface implemented by instances that don't embed
// mx.WithRouter[T].
type Unrouted interface {
	routedBy(if_youre_seeing_this_you_probably_forgot_to_run_mx_generate)
}

var _ Unrouted = (*implementsImpl)(nil)

// AutoMarshal is a type that can be embedded within a struct to indicate that
// "mx generate" should generate serialization methods for the struct.
//
...
//	}
//
// The AutoMarshal embedding instructs "mx generate" to generate
...
// For example, "mx generate" will raise an error for the following code
