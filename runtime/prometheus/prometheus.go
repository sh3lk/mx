...
// TranslateMetricsToPrometheusTextFormat translates MX
// metrics (keyed by mxn id) to a text format that can be
...
func TranslateMetricsToPrometheusTextFormat(w *bytes.Buffer, ms []*metrics.MetricSnapshot, lisAddr, path string) {
	...
	// Display the user metrics first, followed by the MX
	// metrics. Otherwise, the user's metrics can get buried within
	// the MX metrics.
	mxMetrics := map[string][]*metrics.MetricSnapshot{}
	userMetrics := map[string][]*metrics.MetricSnapshot{}
	for _, m := range ms {
		if strings.HasPrefix(m.Name, "mx_") {
			mxMetrics[m.Name] = append(mxMetrics[m.Name], m)
		} else {
			userMetrics[m.Name] = append(userMetrics[m.Name], m)
		}
	}
	sortedUserMetrics := maps.Keys(userMetrics)
	sortedMXMetrics := maps.Keys(mxMetrics)
	sort.Strings(sortedUserMetrics)
	sort.Strings(sortedMXMetrics)
	...
	if len(mxMetrics) > 0 {
		fmt.Fprintf(w, "# ┌─────────────────────────────────────┐\n")
		fmt.Fprintf(w, "# │ MX AUTOGENERATED METRICS │\n")
		fmt.Fprintf(w, "# └─────────────────────────────────────┘\n\n")
	}
	for _, m := range sortedMXMetrics {
		translateMetrics(w, mxMetrics[m])
	}
}
...
// translateMetrics translates a slice of metrics from the MX format
...
func translateMetrics(w *bytes.Buffer, metrics []*metrics.MetricSnapshot) string {
	...
	for idx, metric := range metrics {
		...
		delete(labels, "mx_app")
		delete(labels, "mx_version")
		if node, ok := labels["mx_node"]; ok {
			labels["mx_node"] = logging.Shorten(node)
		}
