...
// Register registers a MX component.
...
// registry is a repository for registered MX components.
// Entries are typically added to the default registry by calls
// to Register in init functions in code generated by "mx generate".
...
// Registration is the configuration needed to register a MX component.
type Registration struct {
	...
	Listeners []string     // the names of any mx.Listeners
...
// register registers a MX component. If the registry's close method was
...
func ComponentConfigValidator(path, cfg string) error {
	...
	if componentConfig == nil {
		return fmt.Errorf("unexpected configuration for component %v "+
			...
			"mx.WithConfig[configType] embedded field to %v)",
...
func CallGraph() []CallEdge {
	...
	for _, reg := range Registered() {
		...
		for i, n := 0, impl.NumField(); i < n; i++ {
			// Handle field with type mx.Ref[T].
			ref := impl.Field(i).Type
			if ref.PkgPath() == "github.com/sh3lk/mx" &&
