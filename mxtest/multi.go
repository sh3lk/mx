...
func initMultiProcess(ctx context.Context, t testing.TB, isBench bool, runner Runner, locals []reflect.Type, logWriter func(*protos.LogEntry), opts mx.RemoteMXNOptions) (*mx.RemoteMXN, func() error, error) {
	t.Helper()
	...
	if bootstrap.Exists() {
		...
		defer func() {
			if r := recover(); r != nil {
				fmt.Fprintf(os.Stderr, "panic in MX sub-process: %v\n", r)
			} else {
				fmt.Fprintf(os.Stderr, "MX sub-process exiting\n")
			}
			os.Exit(1)
		}()

		opts := mx.RemoteMXNOptions{}
		wlet, err := mx.NewRemoteMXN(ctx, codegen.Registered(), bootstrap, opts)
		if err != nil {
			panic(err)
		}
	...
	// Construct AppConfig and MXNArgs.
	...
	mxn, err := d.start(opts)
	if err != nil {
		return nil, nil, err
	}

	return mxn, d.cleanup, nil
}
