...
// deployer is the mxtest multiprocess deployer. Every multiprocess
// mxtest runs its own deployer. The main component is run in the same
// process as the deployer, which is the same process as the unit test. All
// other components are run in subprocesses.
//
// This deployer differs from 'mx multi' in two key ways.
//
...
//     same process as the deployer. This is special to mxtests and
...
type deployer struct {
	...
	wlet       *protos.MXNArgs        // info for subprocesses
...
type group struct {
	...
	controllers []control.MXNControl            // mxn controllers
	components  map[string]bool                 // started components
	addresses   map[string]bool                 // mxn addresses
	subscribers map[string][]control.MXNControl // routing info subscribers, by component
}
...
type handler struct {
	*deployer
	group      *group
	controller control.MXNControl
	subscribed map[string]bool // routing info subscriptions, by component
}

var _ envelope.EnvelopeHandler = &handler{}

// newDeployer returns a new mxtest multiprocess deployer. locals contains
// components that should be co-located with the main component and not
// replicated.
func newDeployer(ctx context.Context, wlet *protos.MXNArgs, config *protos.AppConfig, runner Runner, locals []reflect.Type, logWriter func(*protos.LogEntry), tmpDir string) *deployer {
...
func (d *deployer) start(opts mx.RemoteMXNOptions) (*mx.RemoteMXN, error) {
	...
	// We concurrently start the creation of the mxn and the creation of the envelope
	// connection to the mxn since they both talk to each other.
	type mxnResult struct {
		mxn *mx.RemoteMXN
		err error
	}
	wchan := make(chan mxnResult)
	go func() {
		bootstrap := runtime.Bootstrap{
			Args: child.Args(), // Will block
		}
		mxn, err := mx.NewRemoteMXN(d.ctx, codegen.Registered(), bootstrap, opts)
		wchan <- mxnResult{mxn, err} // Give mxn to caller
		wchan <- mxnResult{mxn, err} // Give mxn to NewEnvelope goroutine
	}()

	// NOTE: NewEnvelope initiates a blocking handshake with the mxn
	// and therefore we run the rest of the initialization in a goroutine which
	// will wait for mx.Run to create a mxn.
	go func() {
		...
		defer d.mu.Unlock()
		g := d.group("main")
		g.controllers = append(g.controllers, w.mxn)
		handler := &handler{
			deployer:   d,
			group:      g,
...
func (d *deployer) startGroup(g *group) error {
	...
	for r := 0; r < DefaultReplication; r++ {
		...
		logger := slog.New(&logging.LogHandler{
			Opts:  logging.Options{Component: "envelope", MXN: wlet.Id},
			Write: d.log,
		})
		e, err := envelope.NewEnvelope(d.ctx, wlet, d.config, envelope.Options{
			Logger: logger,
...
func (d *deployer) group(component string) *group {
	...
	if !ok {
		g = &group{
			...
			subscribers: map[string][]control.MXNControl{},
		}
		d.groups[name] = g
	}
	return g
