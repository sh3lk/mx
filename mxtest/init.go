...
// Runner runs user-supplied testing code as a mx application.
type Runner struct {
	...
	// Config is used as the MX configuration. The
	// string is interpreted as the contents of a MX
...
// AllRunners returns a slice of all builtin mxtest runners.
...
// Test runs a sub-test of t that tests the supplied MX
...
//	...
//
// body is called in the context of a brand-new MX
// application and is passed the *testing.T for the sub-test,
// followed by a the list of components.
//
//	func TestFoo(t *testing.T) {
//		mxtest.Local.Test(t, func(t *testing.T, foo Foo, bar *bar) {
//			// Test foo and bar ...
//		})
//	}
//
...
//
// In contrast with mx.Run, the Test method does not run the Main method of
// any registered mx.Main component.
...
// MX application code. It fails at runtime if body is not a
...
//		mxtest.Local.Bench(b, func(b *testing.B, foo Foo, bar *bar) {
//			for i := 0; i < b.N; i++ {
...
// component implementation pointer types (e.g., *bar).
//
// In contrast with mx.Run, the Bench method does not run the Main method of
// any registered mx.Main component.
...
func (r Runner) sub(t testing.TB, isBench bool, testBody any) {
	...
	if err != nil {
		t.Fatal(fmt.Errorf("mxtest.Run argument: %v", err))
	}
	...
	//
	//     runner.Fakes = append(runner.Fakes, mxtest.Fake[Foo](...))
	...
	if !r.multi && !r.forceRPC {
		...
		var err error
		runner, err = mx.NewSingleMXN(ctx, codegen.Registered(), opts)
		if err != nil {
			t.Fatal(err)
		}
	} else {
		opts := mx.RemoteMXNOptions{Fakes: fakes, InjectRetries: r.injectRetries}
...
// checkRunFunc checks that the type of the function passed to mxtest.Run
...
func checkRunFunc(t testing.TB, fn any) (func(context.Context, mx.MXN) error, []reflect.Type, error) {
	...
	if fnType.In(0) != reflect.TypeOf(t) {
		return nil, nil, fmt.Errorf("function first argument type %v does not match first mxtest.Run argument %T", fnType.In(0), t)
	}
	...
	return func(ctx context.Context, runner mx.MXN) error {
...
// extractComponentInterfaceType on a struct that embeds mx.Implements[Foo]
// returns Foo.
//
...
func extractComponentInterfaceType(t reflect.Type) (reflect.Type, error) {
	...
	// See the definition of mx.Implements.
	f, ok := t.FieldByName("component_interface_type")
	if !ok {
		return nil, fmt.Errorf("type %v does not embed mx.Implements", t)
	}
	return f.Type, nil
}
