...
import (
	...
	"github.com/sh3lk/mx/mxtest/internal/deploy"
)
...
func TestReplicated(t *testing.T) {
	...
	// Instruct the mxtest deployer to replicate each component. Note that each
	// component should be replicated mxtest.DefaultReplication times.
	mxtest.Multi.Test(t, func(t *testing.T, w deploy.Widget) {
		dir := t.TempDir()
		w.Use(ctx, dir)
		// Verify that deployed processes started.
		want := mxtest.DefaultReplication
