...
// started is a MX component that can mark itself as started.
type started struct {
	mx.Implements[Started]
	id uuid.UUID
}
...
// widget is a MX component that deploys started MX components.
type widget struct {
	mx.Implements[Widget]
	started mx.Ref[Started]
}

// Use calls MarkStarted on every replica.
func (w *widget) Use(ctx context.Context, dir string) error {
	// Print to stderr. This tests that mxtest output is captured correctly.
