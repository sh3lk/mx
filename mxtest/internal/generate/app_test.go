...
func TestSuccess(t *testing.T) {
	...
	mxtest.Multi.Test(t, func(t *testing.T, client testApp) {
...
func TestPanic(t *testing.T) {
	t.Skip("mxtest crashes if any component panics, even in another process")
	ctx := context.Background()
	mxtest.Multi.Test(t, func(t *testing.T, client testApp) {
		// Trigger a panic.
		_, err := client.Get(ctx, "foo", panicError)
		if err == nil || !errors.Is(err, mx.RemoteCallError) {
			t.Fatalf("expected a mx.RemoteCallError; got: %v", err)
		}
	})
}
