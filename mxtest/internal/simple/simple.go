...
type source struct {
	mx.Implements[Source]
	dst mx.Ref[Destination]
}
...
var (
	_ mx.NotRetriable = Source.Emit
	_ mx.NotRetriable = Destination.Record
	_ mx.NotRetriable = Destination.RoutedRecord
)
...
type destination struct {
	mx.Implements[Destination]
	mx.WithRouter[destRouter]
	mu       sync.Mutex
...
// Server is a component used to test MX listener handling.
...
func (s *server) Init(ctx context.Context) error {
	...
	s.srv = &http.Server{
		Handler: mx.InstrumentHandlerFunc("test", func(w http.ResponseWriter, r *http.Request) {
