...
// are some situations where it's unclear what a mxn should do.
//
// - A component method call panics.
// - A component's Init method fails.

var (
	componenta = "github.com/sh3lk/mx/internal/testdeployer/a"
	componentb = "github.com/sh3lk/mx/internal/testdeployer/b"
	componentc = "github.com/sh3lk/mx/internal/testdeployer/c"
	componentd = "github.com/sh3lk/mx/internal/testdeployer/d"
	colocated  = map[string][]string{"1": {componenta, componentb, componentc}}
)

// A mxn is a connection to a RemoteMXN running in this process.
type mxn struct {
	cancel  context.CancelFunc // shuts down the mxn
	env     *envelope.Envelope // envelope
	wlet    *mx.RemoteMXN      // mxn
	threads *errgroup.Group    // background threads
}

// spawn spawns a mxn with the provided info and handler.
func spawn(ctx context.Context, info *protos.MXNArgs, handler envelope.EnvelopeHandler, log *slog.Logger, tmpDir string) (*mxn, error) {
	// envelope.NewEnvelope blocks performing a handshake with the mxn, so
	...
	if err != nil {
		cancel()
		return nil, fmt.Errorf("spawn: NewRemoteMXN: %w", err)
	}
	...
	// Monitor the envelope and mxn in background threads. Discard errors
...
// deployer is a simple testing deployer that spawns all mxns in the
// current process.
type deployer struct {
	t         *testing.T         // underlying unit test
	ctx       context.Context    // context used to spawn mxns
	cancel    context.CancelFunc // shuts down the deployer and all mxns
	info      *protos.MXNArgs
	logger    *logging.TestLogger // logger
	threads   *errgroup.Group     // background threads
	placement map[string][]string // mxn -> components
	placedAt  map[string][]string // component -> mxns
	mxns      map[string]*mxn     // mxns
...
// # Placement
//
// placement is a map from mxn names to component names. It describes how
// many mxns to run, what to name them, and which components the mxns
// should host. For example, the following placement creates a single mxn
// that hosts components a, b, and c:
//
//	map[string][]string{"1": {componenta, componentb, componentc}}
//
// The following placement creates three mxns, each with its own
// component:
//
//	map[string][]string{
//	    "1": {componenta},
...
//	}
//
// The following placement replicates a, b, and c across two mxns:
//
//	map[string][]string{
...
func deploy(t *testing.T, ctx context.Context, placement map[string][]string) *deployer {
	return deployWithInfo(t, ctx, placement, &protos.MXNArgs{
		App:             "remotemxn_test.go",
		DeploymentId:    fmt.Sprint(os.Getpid()),
		InternalAddress: "localhost:0",
	})
}

// deployWithInfo is identical to deploy but with an additional MXNArgs
// argument.
func deployWithInfo(t *testing.T, ctx context.Context, placement map[string][]string, info *protos.MXNArgs) *deployer {
	t.Helper()
	...
	for name := range placement {
		...
		mxn, err := spawn(ctx, info, d, logger, tmpDir)
		if err != nil {
			t.Fatal(err)
		}
		d.mxns[name] = mxn
...
// shutdown shuts down a deployer and its mxns.
func (d *deployer) shutdown() {
	d.cancel()
	for _, mxn := range d.mxns {
		if err := mxn.threads.Wait(); err != nil {
			d.t.Fatal(err)
		}
	}
}
...
func (d *deployer) ActivateComponent(ctx context.Context, req *protos.ActivateComponentRequest) (*protos.ActivateComponentReply, error) {
	...
	for _, name := range d.placedAt[req.Component] {
		mxn := d.mxns[name]
		if _, err := mxn.wlet.UpdateComponents(ctx, components); err != nil {
			return nil, err
		}
		replicas = append(replicas, mxn.env.MXNAddress())
	}

	// For simplicity, route locally if there is a single mxn, and route
	// remotely otherwise. We also report routing info to all mxns, not
	...
	for _, mxn := range d.mxns {
		if _, err := mxn.wlet.UpdateRoutingInfo(ctx, routing); err != nil {
			return nil, err
		}
	}
...
func testComponents(dep *deployer) {
	...
	for _, name := range dep.placedAt[componenta] {
		x, err := dep.mxns[name].wlet.GetIntf(reflection.Type[a]())
		if err != nil {
			dep.t.Fatal(err)
		}
		const want = 42
	...
	for _, name := range dep.placedAt[componentd] {
		x, err := dep.mxns[name].wlet.GetIntf(reflection.Type[d]())
		if err != nil {
			dep.t.Fatal(err)
		}
...
func TestLocalhostMXNAddress(t *testing.T) {
	// Start the mxn with internal address "localhost:12345".
	d := deployWithInfo(t, context.Background(), colocated, &protos.MXNArgs{
		App:             "remotemxn_test.go",
		DeploymentId:    fmt.Sprint(os.Getpid()),
...
func TestHostnameMXNAddress(t *testing.T) {
	// Start the mxn with internal address "$HOSTNAME:12345".
...
func TestFailActivateComponent(t *testing.T) {
	...
	d.activateComponent = func(ctx context.Context, req *protos.ActivateComponentRequest) (*protos.ActivateComponentReply, error) {
		...
		if _, err := d.mxns["1"].wlet.UpdateRoutingInfo(ctx, routing); err != nil {
			return nil, err
		}
		components := &protos.UpdateComponentsRequest{Components: []string{req.Component}}
		if _, err := d.mxns["1"].wlet.UpdateComponents(ctx, components); err != nil {
			return nil, err
		}
		return &protos.ActivateComponentReply{}, nil
	}
...
func TestGetListenerAddressReturnsAddressAlreadyInUse(t *testing.T) {
	...
	// Tell the mxn to listen on port 45678 a number of times.
	const n = 3
	failures := map[string]int{}
	d.getListenerAddress = func(ctx context.Context, req *protos.GetListenerAddressRequest) (*protos.GetListenerAddressReply, error) {
		if failures[req.Name] < n {
...
func TestUpdateMissingComponents(t *testing.T) {
	...
	// Update the mxn with components that don't exist.
	components := &protos.UpdateComponentsRequest{Components: []string{"foo", "bar"}}
	if _, err := d.mxns["1"].wlet.UpdateComponents(context.Background(), components); err == nil {
...
func TestUpdateExistingComponents(t *testing.T) {
	...
	// Update the mxn with components that have already been started.
...
func TestUpdateNilRoutingInfo(t *testing.T) {
	...
	// Update the mxn with a nil routing info.
	routing := &protos.UpdateRoutingInfoRequest{}
	if _, err := d.mxns["1"].wlet.UpdateRoutingInfo(ctx, routing); err == nil {
...
func TestUpdateRoutingInfoMissingComponent(t *testing.T) {
	...
	// Update the mxn with routing info for a component that doesn't
	// exist.
	routing := &protos.UpdateRoutingInfoRequest{
...
func TestUpdateRoutingInfoNotStartedComponent(t *testing.T) {
	...
	// Update the mxn with routing info for a component that has hasn't
	// started yet.
	routing := &protos.UpdateRoutingInfoRequest{
		RoutingInfo: &protos.RoutingInfo{
...
func TestUpdateLocalRoutingInfoWithNonLocal(t *testing.T) {
	...
	// Update the mxn with non-local routing info for a component, even
...
func TestFailReplica(t *testing.T) {
	...
	// Kill replica 3.
	d.mxns["3"].cancel()
	if err := d.mxns["3"].threads.Wait(); err != nil {
		t.Fatal(err)
	}
	...
	for _, wlet := range []string{"1", "2"} {
		for _, component := range []string{componentb, componentc} {
			routing := &protos.UpdateRoutingInfoRequest{
				RoutingInfo: &protos.RoutingInfo{
					Component: component,
					Replicas:  []string{d.mxns["2"].env.MXNAddress()},
				},
			}
			if _, err := d.mxns[wlet].wlet.UpdateRoutingInfo(ctx, routing); err != nil {
				t.Fatal(err)
			}
		}
	}
...
func TestUpdateBadRoutingInfo(t *testing.T) {
	...
	d.activateComponent = func(ctx context.Context, req *protos.ActivateComponentRequest) (*protos.ActivateComponentReply, error) {
		...
		routing.RoutingInfo.Replicas = []string{mxn.env.MXNAddress()}
		go func() {
