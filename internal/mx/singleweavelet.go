...
// SingleMXNOptions configure a SingleMXN.
type SingleMXNOptions struct {
...
// SingleMXN is a mxn that runs all components locally in a single
// process. It is the mxn used when you "go run" a MX app.
type SingleMXN struct {
	...
	opts         SingleMXNOptions     // options
	config       *single.SingleConfig // "[single]" section of config file
	deploymentId string               // globally unique deployment id
	id           string               // globally unique mxn id
	mxInfo       *MXInfo              // application's runtime information
	createdAt    time.Time            // time at which the mxn was created
...
// NewSingleMXN returns a new SingleMXN that hosts the components
// specified in the provided registrations.
func NewSingleMXN(ctx context.Context, regs []*codegen.Registration, opts SingleMXNOptions) (*SingleMXN, error) {
	// Parse config.
...
func (w *SingleMXN) GetIntf(t reflect.Type) (any, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *SingleMXN) GetImpl(t reflect.Type) (any, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	return w.getImpl(t)
}
...
func (w *SingleMXN) getIntf(t reflect.Type, requester string) (any, error) {
	reg, ok := w.regsByIntf[t]
	if !ok {
		return nil, fmt.Errorf("component %v not found; maybe you forgot to run mx generate", t)
	}
	c, err := w.get(reg)
	if err != nil {
		return nil, err
...
func (w *SingleMXN) getImpl(t reflect.Type) (any, error) {
	...
	if !ok {
		return nil, fmt.Errorf("component implementation %v not found; maybe you forgot to run mx generate", t)
	}
	return w.get(reg)
}
...
//
// REQUIRES: w.mu is held.
func (w *SingleMXN) get(reg *codegen.Registration) (any, error) {
...
//
// REQUIRES: w.mu is held.
func (w *SingleMXN) listener(name string) (net.Listener, error) {
...
func (w *SingleMXN) logger(name string) *slog.Logger {
...
func (w *SingleMXN) ServeStatus(ctx context.Context) error {
...
func (w *SingleMXN) Status(context.Context) (*status.Status, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	pid := int64(os.Getpid())
	...
	for component := range w.components {
		...
		c.Replicas = append(c.Replicas, &status.Replica{Pid: pid, MXNId: w.id})
...
func (w *SingleMXN) Metrics(context.Context) (*status.Metrics, error) {
	m := &status.Metrics{}
	for _, snap := range metrics.Snapshot() {
		...
		proto.Labels["mx_app"] = w.config.App.Name
		proto.Labels["mx_version"] = w.deploymentId
		proto.Labels["mx_node"] = w.id
...
func (w *SingleMXN) Profile(ctx context.Context, req *protos.GetProfileRequest) (*protos.GetProfileReply, error) {
