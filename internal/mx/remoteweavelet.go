...
// RemoteMXNOptions configure a RemoteMXN.
type RemoteMXNOptions struct {
...
// RemoteMXN is a mxn that runs some components locally, but
// coordinates with a deployer over a set of Unix pipes to start other
// components remotely. It is the mxn used by all deployers, except for
// the single process deployer.
//
// RemoteMXN must implement the mx.controller component interface.
type RemoteMXN struct {
	ctx       context.Context         // shuts down the mxn when canceled
	servers   *errgroup.Group         // background servers
	opts      RemoteMXNOptions        // options
	args      *protos.MXNArgs         // info from envelope
	dialAddr  string                  // Address dialed by other components
	id        string                  // unique id for this mxn
	mxInfo    *MXInfo                 // application runtime information
...
var _ control.MXNControl = (*RemoteMXN)(nil)
...
// component represents a MX component and all corresponding
// metadata.
...
// NewRemoteMXN returns a new RemoteMXN that hosts the components
// specified in the provided registrations. bootstrap is used to establish a
// connection with an envelope.
func NewRemoteMXN(ctx context.Context, regs []*codegen.Registration, bootstrap runtime.Bootstrap, opts RemoteMXNOptions) (*RemoteMXN, error) {
	args := bootstrap.Args
	if args == nil {
		err := fmt.Errorf("missing MXNArgs")
		return nil, err
	}
	if err := runtime.CheckMXNArgs(args); err != nil {
		return nil, err
	}
	...
	// Pre-construct reply for InitMXN call.
	...
	w := &RemoteMXN{
		...
		args:             args,
		dialAddr:         dialAddr,
		mxInfo:           &MXInfo{DeploymentID: args.DeploymentId},
	...
	// Set up logging.
	w.syslogger = w.logger("mxn", "mx/system", "")
	...
	// Serve RPC requests from other mxns.
	...
	w.syslogger.Debug("🧶 mxn started", "addr", dialAddr)
	return w, nil
}

// InitMXN implements mx.controller and conn.MXHandler interfaces.
func (w *RemoteMXN) InitMXN(ctx context.Context, req *protos.InitMXNRequest) (*protos.InitMXNReply, error) {
	w.initMu.Lock()
...
// Wait waits for the RemoteMXN to fully shut down after its context has
// been cancelled.
func (w *RemoteMXN) Wait() error {
	return w.servers.Wait()
}

// GetIntf implements the MXN interface.
func (w *RemoteMXN) GetIntf(t reflect.Type) (any, error) {
...
func (w *RemoteMXN) getIntf(t reflect.Type, requester string) (any, error) {
	c, ok := w.componentsByIntf[t]
	if !ok {
		return nil, fmt.Errorf("component of type %v was not registered; maybe you forgot to run mx generate", t)
	}
...
func (w *RemoteMXN) redirect(requester string, c *component, target, address string) (any, error) {
...
// GetImpl implements the MXN interface.
func (w *RemoteMXN) GetImpl(t reflect.Type) (any, error) {
	c, ok := w.componentsByImpl[t]
	if !ok {
		return nil, fmt.Errorf("component implementation of type %v was not registered; maybe you forgot to run mx generate", t)
	}
...
//
// TODO(mwhittaker): Deduplicate with localmxn.go.
func (w *RemoteMXN) createComponent(ctx context.Context, reg *codegen.Registration) (any, error) {
	...
	if err := SetMXInfo(obj, w.mxInfo); err != nil {
		return nil, err
	}
...
func (w *RemoteMXN) getStub(c *component) (codegen.Stub, error) {
	c.stubInit.Do(func() {
...
func (w *RemoteMXN) makeStub(fullName string, reg *codegen.Registration, resolver call.Resolver, balancer call.Balancer, wait bool) (codegen.Stub, error) {
...
func (w *RemoteMXN) GetLoad(context.Context, *protos.GetLoadRequest) (*protos.GetLoadReply, error) {
...
// UpdateComponents implements mx.controller and conn.MXHandler interfaces.
func (w *RemoteMXN) UpdateComponents(_ context.Context, req *protos.UpdateComponentsRequest) (*protos.UpdateComponentsReply, error) {
	var errs []error
...
func (w *RemoteMXN) UpdateRoutingInfo(_ context.Context, req *protos.UpdateRoutingInfoRequest) (reply *protos.UpdateRoutingInfoReply, err error) {
...
func (w *RemoteMXN) GetHealth(context.Context, *protos.GetHealthRequest) (*protos.GetHealthReply, error) {
...
func (w *RemoteMXN) GetMetrics(context.Context, *protos.GetMetricsRequest) (*protos.GetMetricsReply, error) {
	...
	// Add mxn labels to the metrics.
	for _, def := range updates.Defs {
		if def.Labels == nil {
			def.Labels = map[string]string{}
		}
		def.Labels["mx_app"] = w.Info().App
		def.Labels["mx_version"] = w.Info().DeploymentId
		def.Labels["mx_node"] = w.Info().Id
	}
...
func (w *RemoteMXN) GetProfile(ctx context.Context, req *protos.GetProfileRequest) (*protos.GetProfileReply, error) {
...
// Info returns the MXNArgs received from the envelope.
func (w *RemoteMXN) Info() *protos.MXNArgs {
	return w.args
}

// getComponent returns the component with the given name.
func (w *RemoteMXN) getComponent(name string) (*component, error) {
	...
	if !ok {
		return nil, fmt.Errorf("component %q was not registered; maybe you forgot to run mx generate", name)
	}
	return c, nil
}
...
// calls m.
func (w *RemoteMXN) addHandlers(handlers *call.HandlerMap, c *component) {
...
func (w *RemoteMXN) repeatedly(ctx context.Context, errMsg string, f func() error) error {
...
func (w *RemoteMXN) getDeployerControl() (control.DeployerControl, error) {
...
func (w *RemoteMXN) logger(name string, attrs ...string) *slog.Logger {
...
func (w *RemoteMXN) sendTraceSpans(spans *protos.TraceSpans) error {
	select {
...
func (w *RemoteMXN) listener(ctx context.Context, name string) (*listener, error) {
	w.lismu.Lock()
...
func (w *RemoteMXN) getListenerAddress(ctx context.Context, name string) (string, error) {
...
func (w *RemoteMXN) getSelfCertificate() (*tls.Certificate, error) {
...
func (w *RemoteMXN) verifyClientCertificate(certChain [][]byte) ([]string, error) {
...
func (w *RemoteMXN) verifyServerCertificate(certChain [][]byte, targetComponent string) error {
...
// server serves RPC traffic from other RemoteMXNs.
type server struct {
	net.Listener
	wlet *RemoteMXN
}
