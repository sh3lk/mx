...
// loadCollector collects load for a MX component. As an example, imagine we
// have a load collector lc for a MX component that owns slices [0, 10) and
...
func (lc *loadCollector) add(key uint64, v float64) error {
	...
	if !found {
		...
		// different mxn may have an older or newer version of the current
...
func newSliceSummary(slice slice) (*sliceSummary, error) {
	...
	// about a kilobyte of memory. Given that a mxn should manage a
