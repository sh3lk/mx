...
const (
	...
	babysitterInfoKey = "MX_BABYSITTER_INFO"
)

var (
	// The directories and files where "mx ssh" stores data.
	//
...
type manager struct {
	...
	// to store the mx binaries at each location
	locations map[string]string

	// logSaver processes log entries generated by the mxns and babysitters.
	// The entries either have the timestamp produced by the mxn/babysitter,
	...
	// traceSaver processes trace spans generated by the mxn. If nil,
	// mxn traces are dropped.
	//
...
func (m *manager) registerReplica(_ context.Context, req *ReplicaToRegister) error {
	...
	record := func() bool {
		...
		g.replicas = append(g.replicas, &status.Replica{Pid: req.Pid, MXNId: req.MXNId})
		return false
	}
	if record() {
		return nil
...
func (m *manager) startBabysitter(loc string, info *BabysitterInfo) error {
	...
	binaryPath := filepath.Join(m.locations[loc], "mx")
...
// $XDG_DATA_HOME/mx/ssh/registry, or
// ~/.local/share/mx/ssh/registry if XDG_DATA_HOME is not set.
